...
const AccountMenu: React.FC<AccountMenuProps> = ({ onNavigate }) => {
  ...
  return (
    <div ref={rootRef} className="relative space-y-3">
      {/* Daily points — always visible, matches the app's light card surfaces */}
      <div className="rounded-xl border border-gray-200 bg-gray-50/70 p-3">
        <div className="flex items-center justify-between">
          <div className="min-w-0">
            <div className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">Your points</div>
            <div className="mt-0.5 flex items-baseline gap-1.5">
              <span className="text-xl font-bold text-gray-900 tabular-nums">{fmt(pointTotal)}</span>
              {streak > 0 && (
        ...
        <div className="mt-2.5">
          {!address ? (
            <button
              ...
              className="w-full py-2 rounded-lg bg-gray-900 hover:bg-black text-white text-xs font-semibold transition-colors"
            >
            ...
            <button
              onClick={handleClaim}
              disabled={claiming}
              className="w-full py-2 rounded-lg bg-gray-900 hover:bg-black text-white text-xs font-semibold transition-colors disabled:opacity-70 flex items-center justify-center gap-2"
            >
              {claiming ? (
                <>
            ...
            <div className="w-full py-1.5 rounded-lg bg-gray-100 text-center">
              <div className="text-[10px] text-gray-400">Next claim in</div>
              <div className="text-sm font-bold tabular-nums font-mono text-gray-700">{formatCountdown(cooldownMs)}</div>
            </div>
          )}
          {claimFlash && <p className="mt-2 text-center text-xs font-semibold text-emerald-600">{claimFlash}</p>}
          {claimError && <p className="mt-2 text-center text-xs text-rose-500">{claimError}</p>}
        </div>
      </div>
        ...
        >
          <div className="w-9 h-9 bg-gray-200 rounded-full flex items-center justify-center overflow-hidden flex-shrink-0">
            {hasCustomAvatar ? (
              ...
              <span className="text-sm font-bold text-gray-600">{displayName?.[0]?.toUpperCase() || 'U'}</span>
            )}
          </div>
